...
export async function GET(
  ...
  const pdfBuffer = await new Promise<Buffer>((resolve, reject) => {
    ...
    // 1. Header Title
    doc.fillColor(primaryColor).fontSize(20).font("Helvetica-Bold").text("KERALA STATE LOTTERY RESULTS", { align: "center" });
    doc.fontSize(10).font("Helvetica").fillColor(lightText).text("Unofficial Result Gazette Sheet • keralalotteryresultstoday.in", { align: "center" });
    doc.moveDown(1.5);

    // 2. Draw Info Panel
    const infoY = doc.y;
    doc.rect(40, infoY, 515, 60).fill(greyBg);
    doc.fillColor(darkText).font("Helvetica-Bold").fontSize(11);
    doc.text(`Lottery Draw: ${result.draw_name} (${result.draw_code})`, 55, infoY + 14);
    doc.text(`Draw Date: ${result.draw_date}`, 55, infoY + 34);
    doc.y = infoY + 60; // Advance cursor
    doc.moveDown(1.2);

    // 3. First Prize Block
    const firstPrizeY = doc.y;
    doc.rect(40, firstPrizeY, 515, 80).fill(primaryColor);
    
    doc.fillColor("#FFFFFF").font("Helvetica-Bold").fontSize(11).text("FIRST PRIZE WINNER", 55, firstPrizeY + 12);
    
    const ticketNo = result.first?.ticket || "N/A";
    doc.fillColor("#FBBF24").font("Helvetica-Bold").fontSize(22).text(ticketNo, 55, firstPrizeY + 26);
    
    const location = result.first?.location || "N/A";
    const agent = result.first?.agent || "N/A";
    const agencyNo = result.first?.agency_no || "N/A";
    doc.fillColor("#E2E8F0").font("Helvetica").fontSize(9).text(`Agent: ${agent} (${agencyNo}) | Location: ${location}`, 55, firstPrizeY + 56);
    
    doc.y = firstPrizeY + 80;
    doc.moveDown(1.5);

    // 4. Remaining Prize Tiers
    doc.fillColor(primaryColor).fontSize(14).font("Helvetica-Bold").text("PRIZE DRAW DETAILS", 40, doc.y);
    doc.moveDown(0.5);
    ...
    for (const [tier, numbers] of Object.entries(prizes)) {
      if (Array.isArray(numbers) && numbers.length > 0) {
        ...
        doc.fillColor(primaryColor).fontSize(11).font("Helvetica-Bold").text(`${tier} Prize:`, 40, doc.y);
        doc.moveDown(0.25);
        doc.fillColor(darkText).fontSize(9).font("Helvetica").text(numbers.join(", "), 40, doc.y, { width: 515 });
        doc.moveDown(0.8);
      }
    }
